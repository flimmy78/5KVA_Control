...
float sin_theta1 = 0, cos_theta1 = 0, sin_theta2 = 0, cos_theta2 = 0,
		sin_theta3 = 0, cos_theta3 = 0;
...
void main(void) {
...

	{
		...
		if (Offset_done == 0)			//Wait for ADC offset complete
			...
			if ((Ia_abs > Ipeak_limit) | (Ib_abs > Ipeak_limit)
					| (Ic_abs > Ipeak_limit))      //trip if overcurrent 50A
					{
				Turn_off_Converter();
			}
...
void Offset_ADC() {
	...
	for (kk = 0; kk < 10; kk++) {
		ADC_Result[kk] = (DMA_Buf1[kk] + DMA_Buf1[kk + 10] + DMA_Buf1[kk + 20]
				+ DMA_Buf1[kk + 30] + DMA_Buf1[kk + 40]) * 0.2;
...
void ADC_Conversion() {
	...
	for (k = 0; k < 8; k++) {
		ADC_Result[k] = (DMA_Buf1[k] + DMA_Buf1[k + 10] + DMA_Buf1[k + 20]
				+ DMA_Buf1[k + 30] + DMA_Buf1[k + 40]) * 0.2;
	...
	SampRes1.DCBusVol_1n = Kdc_samp1
			* ((float) ADC_Result[3] - OffSetRes.OS_DCBusVol_1);
	SampRes1.DCBusVol_2n = Kdc_samp2
			* ((float) ADC_Result[4] - OffSetRes.OS_DCBusVol_2);
