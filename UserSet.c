...
/*==========================================================================================
 采样pin为ADCA0~3，ADCB0~5共十个采样量，连续采样。EWPM4启动ADC，ADC结束启动DMA传递数据
===========================================================================================*/ 
...
//DB死区模块设定
  ...
  /* EPwm2Regs.DBCTL.bit.IN_MODE  = DBA_ALL;
   ...
   EPwm2Regs.DBRED = DeadTime;                        // 上升沿延迟，死区时间设为2us
   EPwm2Regs.DBFED = DeadTime;                        // 下降沿延迟，死区时间设为2us

   EPwm6Regs.DBCTL.bit.IN_MODE  = DBA_ALL;
   EPwm6Regs.DBCTL.bit.POLSEL   = DB_ACTV_HIC;
   EPwm6Regs.DBCTL.bit.OUT_MODE = DB_FULL_ENABLE;
   EPwm6Regs.DBRED = DeadTime;                        // 上升沿延迟，死区时间设为2us
   EPwm6Regs.DBFED = DeadTime;                        // 下降沿延迟，死区时间设为2us*/

 //  EPwm5Regs.DBCTL.bit.OUT_MODE = 0;//#################

//TZ模块设定，完成
   EALLOW;
   EPwm1Regs.TZSEL.bit.CBC1 = 1;                  //TZ1为触发源
   EPwm2Regs.TZSEL.bit.CBC1 = 1;
   EPwm6Regs.TZSEL.bit.CBC1 = 1;
  // EPwm5Regs.TZSEL.bit.CBC1 = 1;  //##########

   EPwm1Regs.TZCTL.bit.TZA = 3;                  //事件发生，EPWMA输出置低
...
//事件设定
   ...
   */
   ...
   EPwm4Regs.ETSEL.bit.SOCASEL = ET_CTR_ZERO;     //ZERO 触发ADC_A
...
void UserSetCana()
{
  ...
  EALLOW;
  ECanaRegs.CANMIL.all = 0x00000000;             //邮箱中断优先级
...
{
    MainState = FirstStart;           //逆变器状态为第一次启动
    ProFlag.all = 0x0000;        //逆变器状态标志位
    ...
    memset((void *)&SampRes,0,sizeof(SampRes));               //采样寄存器初始化为0
    memset((void *)&GridVoltagePLL,0,sizeof(GridVoltagePLL)); //电网电压锁相电压信息
	GridVoltagePLL.Fre = 50;
    memset((void *)&BriVolDQ,0,sizeof(BriVolDQ));             //桥臂输出电压信息
	BriVolDQ.Fre = 50;
    memset((void *)&BriVolPID_D,0,sizeof(BriVolPID_D));       //桥臂电压D轴PID
    memset((void *)&BriVolPID_Q,0,sizeof(BriVolPID_Q));       //桥臂电压Q轴PID
    memset((void *)&IndCurPID_D,0,sizeof(IndCurPID_D));       //输出电感电流D轴PID
    memset((void *)&IndCurPID_Q,0,sizeof(IndCurPID_Q));       //输出电感电流Q轴PID
    memset((void *)&SPWMGen,0,sizeof(SPWMGen));               //SPWM输出信息寄存器
	SPWMGen.Fre = 50;
    memset((void *)&CurReference,0,sizeof(CurReference));     //输出电流指令
    CurReference.Fre = 50;
    memset((void *)&IndCurDQ,0,sizeof(IndCurDQ));             //实测输出电感电流
    IndCurDQ.Fre = 50;
    memset((void *)&VoltageControl,0,sizeof(VoltageControl));
	memset((void *)&FuncCounter,0,sizeof(FuncCounter));       // 函数计数器
    memset((void *)&OutVolLoopPID,0,sizeof(OutVolLoopPID));  //电压外环PID
    memset((void *)&FrePowerDroop,0,sizeof(FrePowerDroop));
	memset((void *)&VolRePowerDrp,0,sizeof(VolRePowerDrp));
    memset((void *)&DeadTime_Res,0,sizeof(DeadTime_Res));    //死区补偿寄存器
