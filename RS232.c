...
Uint16 GUIdata[12]= {3805, 5000, 10000, 0, 500, 495, 505, 1203, 1154, 1256, 0, 0};
void init_SCI(void){
	Uint16 i ;
	//Init GPIO
	EALLOW;
...
void rx_RS232(RS232Obj *RS232){
	...
	//	}
		...
		//read data
		while(RxNum>0) {
			RS232->RxBuf[RS232->RxCnt] = ((((Uint16)SciaRegs.SCIRXBUF.bit.RXDT)<<8) & 0xff00 ) ; //Hi Byte
			RS232->RxBuf[RS232->RxCnt] = (SciaRegs.SCIRXBUF.bit.RXDT & 0x00ff) | (RS232->RxBuf[RS232->RxCnt]) ;;	//Low byte
			RS232->RxCnt ++ ;
			RxNum-- ;
		}
	}
          ...
          switch(TempWord.bits.Lo)
			{
				case 0x01:              // Receive data in bus monitor mode
				  ...
				  if(Rxbuf[2] & 0x8000)
				    {
				     GridReactiveCur= (((Rxbuf[2]>>8) & 0x00ff-128) * 256 + (Rxbuf[2] & 0x00ff))/(-100.0);
				    }
				  else
                    {
			    ...
			    case 0x02:              // Receive data in power dispatch mode
	              ...
	              if(Rxbuf[1] & 0x8000)
				    {
				     GridActiveCur = (((Rxbuf[1]>>8) & 0x00ff-128) * 256 + (Rxbuf[1] & 0x00ff))/(-100.0);
				    }
				  else
                    {
